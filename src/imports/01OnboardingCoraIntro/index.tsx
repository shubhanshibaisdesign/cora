...
function AppleButton({ label = "Start Pairing", onClick }: { label?: string; onClick?: () => void }) {
  return (
    <div className="absolute bg-[#dd692c] h-[52px] left-0 overflow-clip rounded-[41px] top-[693px] w-[353px] cursor-pointer" onClick={onClick} data-name="Apple Button">
...
function ContentArea({ onComplete, onSupportingClick }: { onComplete?: () => void; onSupportingClick?: () => void }) {
  ...
  const handleClick = () => {
    if (phase === "idle") startPairing();
  };

  return (
    <div className="h-[774px] relative shrink-0 w-full" data-name="ContentArea">
      <AppleButton label={phase === "connected" ? "Device Connected" : "Start Pairing"} onClick={handleClick} />

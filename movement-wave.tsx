...
const peaks = [0.03, 0.08, 0.35, 1.0, 0.7, 0.25, 0.12, 0.08, 0.15, 0.4, 0.25, 0.1, 0.06, 0.12, 0.25, 0.15, 0.08, 0.05, 0.1, 0.18, 0.12, 0.06, 0.04, 0.03];
...
export function MovementWave() {
  ...
  useEffect(() => {
    ...
    const camera = new THREE.OrthographicCamera(-viewW / 2, viewW / 2, viewH, -0.05, 0.1, 10);
    ...
    for (let i = 0; i < OUTLINE_PTS; i++) {
      ...
      const y = getHeight(xNorm) * viewH * 0.9;
    ...
    for (let i = 0; i < FILL_PTS; i++) {
      ...
      const x = (xNorm - 0.5) * viewW;
      const curveY = getHeight(xNorm) * viewH * 0.9;
      ...
      const heightRatio = curveY > 0 ? y / curveY : 0;
      const c = heightRatio > 0.7 ? c3.clone() : heightRatio > 0.4 ? c2.clone() : c1.clone();
    ...
    for (let i = 0; i < PEAK_FILL; i++) {
      const xNorm = 0.06 + Math.random() * 0.22;
      const x = (xNorm - 0.5) * viewW;
      const curveY = getHeight(xNorm) * viewH * 0.9;
      const y = Math.random() * curveY;
